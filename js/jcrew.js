...
function request_page(url, callback){
    ...
    page.onLoadFinished = function (status) {
        ...
        var item = page.evaluate(function () {
            ...
            }else if($('div.full-price span.price-soldout').length > 0){
            	itemObject.originalPrice = $('div.full-price span.price-soldout').text().trim().replace('$','');
			}else{
			...
			if($('div.categoryPromoPDP').length > 0){
				console.log("hhhhhhhhhhhhhhhhhh");
				var prom = $('div.categoryPromoPDP').text().match('Extra ([0-9]+)% off with code ([A-Z]+)', "$2, $1")
				var itemPromotion = new Object();
				itemPromotion.discount = prom[1]/100;
				itemPromotion.promotion = new Object();
				itemPromotion.promotion.promotionCode = prom[2];
				itemObject.itemPromotions.push(itemPromotion);
			}else{
